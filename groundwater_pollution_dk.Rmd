...
#okologimark <- as_Spatial(okologimark)
...
okologimark_merged_geo <- c(okologimark_2012$geometry,okologimark_2013$geometry, okologimark_2014$geometry, okologimark_2015$geometry, okologimark_2016$geometry, okologimark_2017$geometry, okologimark_2018$geometry, okologimark_2019$geometry, okologimark_2020$geometry)

okologimark <- as_Spatial(okologimark_merged_geo)

okologimark_merged_geo <- as.data.frame(okologimark_merged_geo)

markblok<-markblok$geometry

markblok<-as.data.frame(markblok)

markblok<-rbind(markblok,okologimark_merged_geo)

markblok<-markblok[!duplicated(markblok$geometry), ]
...
```{r}
# TEST
point <- matrix(c(480258.4, 6178223,
 3.2, 1.75,
 2,3,
 1.5, 1.25, 
...
```{r}
oko_test <- lapply(Adash@polygons[0:10000], function(x) SpatialPolygons(list(x)))
#oko_test2<-oko_test[0:1000]
```
...
shit<-lapply(oko_test, function(x) over(points, x))

shit[74]

which(sapply(shit, function(y) 1 %in% y))


flat_list <- as.data.frame(do.call(rbind, lapply(shit, as.data.frame)))
...
nitrate$land_type[noquote(indexes[1])]
...
nitrate$land_type <- as.factor(nitrate$land_type)

#
p <- ggplot(nitrate, aes(Indtag_topdybde, colour = land_type))
